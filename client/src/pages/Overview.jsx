...
function Overview({ allServerMetrics, connected }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const [serverList, count] = await Promise.all([
          fetchServers(),
    ...
    load();
    // Refresh server list every 15 seconds
    const interval = setInterval(load, 15000);
    return () => { mounted = false; clearInterval(interval); };
  }, []);

  const getLatestMetrics = (serverId) => allServerMetrics[serverId] || {};
      ...
      {servers.length === 0 ? (
        <div className="bg-[#0d1117] rounded-xl border border-gray-800 p-12 text-center">
          ...
          <div className="bg-gray-900 rounded-lg p-4 max-w-md mx-auto text-left">
            ...
            <code className="text-xs text-emerald-400 block">
              API_KEY=your-key npm start --prefix agent
            </code>
          </div>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {servers.map((server) => {
            ...
            const memPct = live.memoryPercent ?? (live.totalMem ? ((live.totalMem - live.freeMem) / live.totalMem) * 100 : 0);
