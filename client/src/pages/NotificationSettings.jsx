...
function NotificationSettings() {
  ...
  async function handleCreate(e) {
    ...
    const config = formType === "slack"
      ...
      : {
          ...
          smtpUser: formSmtpUser.trim(),
          smtpPass: formSmtpPass,
          from: formFrom.trim(),
          to: formTo.trim(),
        };

    if (formType === "slack" && !config.webhookUrl) {
      toast.error("Webhook URL is required");
      return;
    }
    if (formType === "email" && (!config.smtpHost || !config.to)) {
      toast.error("SMTP Host and To address are required");
      return;
    }
  ...
  async function handleToggle(channelId) {
    try {
      const updated = await toggleNotificationChannel(channelId);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <FormField label="Channel Name">
                <input
                  ...
                  placeholder="e.g., Dev Team Alerts"
            ...
            {formType === "slack" ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                <FormField label="SMTP Host">
                  <input type="text" value={formSmtpHost} onChange={(e) => setFormSmtpHost(e.target.value)} placeholder="smtp.gmail.com" className="form-input" required />
                </FormField>
                <FormField label="SMTP Port">
                  <input type="number" value={formSmtpPort} onChange={(e) => setFormSmtpPort(e.target.value)} className="form-input" />
                </FormField>
                <FormField label="SMTP User">
                  <input type="text" value={formSmtpUser} onChange={(e) => setFormSmtpUser(e.target.value)} placeholder="user@gmail.com" className="form-input" />
                </FormField>
                <FormField label="SMTP Password">
                  <input type="password" value={formSmtpPass} onChange={(e) => setFormSmtpPass(e.target.value)} className="form-input" />
                </FormField>
                <FormField label="From Address">
                  <input type="email" value={formFrom} onChange={(e) => setFormFrom(e.target.value)} placeholder="alerts@mycompany.com" className="form-input" />
                </FormField>
                <FormField label="To Address">
                  <input type="email" value={formTo} onChange={(e) => setFormTo(e.target.value)} placeholder="team@mycompany.com" className="form-input" required />
                </FormField>
              </div>
            )}
    ...
    <div className={`bg-[#0d1117] rounded-xl border border-gray-800 p-4 flex items-center gap-4 transition-opacity ${!channel.isActive ? "opacity-50" : ""}`}>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] px-1.5 py-0.5 rounded font-medium bg-gray-700 text-gray-400 uppercase">
            {channel.type}
          </span>
        </div>
        <p className="text-xs text-gray-500 mt-0.5 truncate">
          {isSlack ? channel.config?.webhookUrl?.replace(/\/T[^/]+\/B[^/]+\/.*/, "/T.../B.../...") : `${channel.config?.smtpHost}:${channel.config?.smtpPort} → ${channel.config?.to}`}
        </p>
      </div>
