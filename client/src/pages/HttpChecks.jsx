...
import { Globe, Plus, Trash2, ToggleLeft, ToggleRight, Clock, CheckCircle, XCircle, ShieldCheck, Activity } from "lucide-react";
...
function HttpChecks({ httpCheckResults = {} }) {
  ...
  // Merge live socket results into checks
  useEffect(() => {
    ...
    setChecks((prev) =>
      prev.map((check) => {
        ...
        return {
          ...check,
          status: live.status,
          lastResponseTime: live.responseTime,
          lastStatusCode: live.statusCode,
          sslExpiry: live.sslDaysRemaining,
          uptimePercent: live.uptimePercent,
  ...
  async function handleCreate(e) {
    ...
    try {
      await createHttpCheck({
        ...
        expectedStatus: Number(formExpectedStatus),
      });
      ...
      toast.error(err.response?.data?.error || "Failed to create check");
    } finally {
      setSaving(false);
    }
  }

  async function handleDelete(checkId) {
    try {
      await deleteHttpCheck(checkId);
      toast.success("Check deleted");
      setChecks((prev) => prev.filter((c) => c._id !== checkId));
    } catch {
  ...
  async function handleToggle(checkId) {
    try {
      const updated = await toggleHttpCheck(checkId);
        ...
        <div className="bg-[#0d1117] rounded-xl border border-gray-800 p-5 mb-6">
          ...
          <form onSubmit={handleCreate} className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
            ...
            <FormField label="Check Interval">
              <select value={formInterval} onChange={(e) => setFormInterval(e.target.value)} className="form-input">
                {INTERVALS.map((i) => (
            ...
            <FormField label="Expected Status">
              <div className="flex gap-2">
                <input
                  ...
                  onChange={(e) => setFormExpectedStatus(e.target.value)}
                  className="form-input flex-1"
                />
                <button
                  type="submit"
                  disabled={saving}
                  className="px-4 py-2 bg-emerald-400 text-gray-900 font-medium text-sm rounded-lg hover:bg-emerald-300 transition-colors disabled:opacity-50 whitespace-nowrap"
                >
                  {saving ? "Adding..." : "Add"}
                </button>
              </div>
...
function CheckCard({ check, onDelete, onToggle }) {
  ...
  return (
    <div
      className={`bg-[#0d1117] rounded-xl border p-4 flex items-center gap-4 transition-opacity ${
        isDown ? "border-red-900/50" : "border-gray-800"
      } ${!check.isActive ? "opacity-50" : ""}`}
    >
      {/* Toggle */}
      ...
      <div className="hidden md:flex items-center gap-4 text-xs text-gray-400">
        ...
        {check.lastResponseTime != null && (
          <div className="flex items-center gap-1" title="Response time">
            <Clock size={12} />
            <span className={check.lastResponseTime > 2000 ? "text-amber-400" : "text-gray-300"}>
        ...
        {!isPending && (
          <div className="flex items-center gap-1" title="Uptime">
            <Activity size={12} />
            <span className={check.uptimePercent < 99 ? "text-amber-400" : "text-gray-300"}>
        ...
        {isHttps && check.sslExpiry != null && (
          <div className="flex items-center gap-1" title="SSL certificate expiry">
            <ShieldCheck size={12} />
            <span className={check.sslExpiry < 14 ? "text-amber-400" : "text-gray-300"}>
              {check.sslExpiry}d
            </span>
          </div>
        )}
        ...
        {check.lastStatusCode != null && (
          <span className={`text-[10px] px-1.5 py-0.5 rounded font-mono ${
            check.lastStatusCode >= 200 && check.lastStatusCode < 300 ? "bg-emerald-400/10 text-emerald-400" :
            check.lastStatusCode >= 400 ? "bg-red-400/10 text-red-400" :
