...
interface DockerContainersProps {
  dockerMetrics?: Record<string, DockerContainerRecord[]>;
}

const STATE_STYLES: Record<string, { dot: string; badge: string }> = {
...
function ContainerCard({ container: c }: { container: DockerContainerRecord }) {
  ...
  return (
    <div className={`bg-[#0d1117] rounded-xl border border-gray-800 p-4 ${c.state !== "running" ? "opacity-60" : ""}`}>
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className={`text-[10px] px-1.5 py-0.5 rounded font-medium ${style.badge}`}>{c.state?.toUpperCase()}</span>
      </div>
      <p className="text-xs text-gray-500 mb-3 truncate">{c.image}</p>
      {c.state === "running" && (
        <div className="grid grid-cols-2 gap-3">
          <MiniMetric label="CPU" value={`${(c.cpuPercent ?? 0).toFixed(1)}%`} warn={(c.cpuPercent ?? 0) > 80} />
          <MiniMetric label="Memory" value={`${(c.memPercent ?? 0).toFixed(1)}%`} warn={(c.memPercent ?? 0) > 80} />
...
function DockerContainers({ dockerMetrics = {} }: DockerContainersProps) {
  ...
  useEffect(() => {
    ...
    const allContainers: DockerContainerRecord[] = [];
    ...
    if (allContainers.length > 0) setContainers(allContainers);
  }, [dockerMetrics]);

  async function loadData() {
    try {
      const data = await fetchDockerContainers();
      setContainers(data as DockerContainerRecord[]);
    } catch {
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {containers.map((c) => (
            <ContainerCard key={c.containerId ?? c._id} container={c} />
          ))}
        </div>
      )}
    </div>
